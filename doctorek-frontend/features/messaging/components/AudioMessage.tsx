...
export function AudioMessage({ mediaUrl, durationSec, mine }: AudioMessageProps) {
  ...
  const barOn = 'bg-[#007DFF]'
  const barOff = mine ? 'bg-[#AFCFF3]' : 'bg-[#C7D5E4]'
  const time = 'text-[#5E7EA8]'

  function renderIcon() {
    if (loading) return <LogoLoader variant="mark" size={16} inverse={!mine} decorative />
    if (playing) return <Pause className="h-4 w-4" fill="currentColor" aria-hidden="true" />
    return <Play className="h-4 w-4" fill="currentColor" aria-hidden="true" />
  }

  const shownSec = playing || current ? current : durationSec

  return (
    <div className="flex min-w-[250px] items-center gap-3 py-0.5 pr-1">
      <button
        type="button"
        onClick={toggle}
