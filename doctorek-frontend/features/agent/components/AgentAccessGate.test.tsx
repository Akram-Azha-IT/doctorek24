...
describe('AgentAccessGate', () => {
  test('propose connexion et inscription au visiteur', () => {
    render(<AgentAccessGate estConnecte={false} />)

    expect(screen.getByRole('link', { name: 'Se connecter pour commencer' })).toHaveAttribute('href', '/login')
  ...
  test('explique le rôle requis à un utilisateur non patient', () => {
    render(<AgentAccessGate estConnecte />)

    expect(screen.getByText('Un compte patient est nécessaire')).toBeInTheDocument()
    expect(screen.getByRole('link', { name: 'Changer de compte' })).toHaveAttribute('href', '/login')
