...
function field(opts: {
  ...
  return `
        ...
        <text class="${blank ? 'dk-s' : 'dk-m'}" x="${x}" y="${y + 32}" font-size="${blank ? 17 : valueSize}" font-weight="${blank ? 500 : 700}" fill="${blank ? '#A9BBD2' : valueColor}" letter-spacing="${blank ? 0 : 1.6}">${blank ? 'Non renseigné' : escapeSvgText(value)}</text>`
}
...
export function buildRectoSvg(
  ...
  const photoBlock = photoUrl
    ? `<image href="${escapeSvgText(photoUrl)}" x="48" y="132" width="168" height="206" clip-path="url(#photo-clip-recto)" preserveAspectRatio="xMidYMid slice"/>`
  ...
  const qrBlock = qrDataUrl
    ? `<image href="${escapeSvgText(qrDataUrl)}" x="686" y="296" width="120" height="120"/>`
    : `<g opacity="0.2">
  ...
  return `
    <svg viewBox="0 0 856 540" style="width:100%;height:100%;display:block;" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif">
      ...
      <g clip-path="url(#card-clip-recto)">
        ...
        <image href="${escapeSvgText(logoUrl)}" x="48" y="32" width="156" height="44" preserveAspectRatio="xMinYMid meet"/>
        ...
        <g clip-path="url(#name-clip-recto)">
          <text class="dk-s" x="256" y="210" font-size="${nameSize}" font-weight="700" fill="${C_NAVY}" letter-spacing="-0.5">${escapeSvgText(fullName)}</text>
        </g>
        ...
        <text class="dk-m" x="256" y="424" font-size="24" font-weight="700" fill="${C_RICH}" letter-spacing="2.4">${escapeSvgText(cardRef)}</text>
        ...
        <image href="${escapeSvgText(logoUrl)}" x="48" y="498" width="92" height="30" preserveAspectRatio="xMinYMid meet" style="filter:brightness(0) invert(1);"/>
  ...
  return `
    <svg viewBox="0 0 856 540" style="width:100%;height:100%;display:block;" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif">
      ...
      <g clip-path="url(#card-clip-verso)">
        ...
        <image href="${escapeSvgText(logoUrl)}" x="48" y="32" width="144" height="42" preserveAspectRatio="xMinYMid meet"/>
        ...
        <text class="dk-m" x="808" y="86" text-anchor="end" font-size="25" font-weight="700" fill="${C_RICH}" letter-spacing="2.6">${escapeSvgText(immatNo)}</text>
